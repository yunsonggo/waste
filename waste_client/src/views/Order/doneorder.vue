<template>
  <div class="cinema_body">
    <Scroller
      ...
      ref="scro"
    >
      <ul v-if="doneOrderList">
        <li class="pullDown">{{ pullDownMsg }}</li>
        <li v-for="(order, index) in doneOrderList" :key="index">
          <div>
      ...
      <div v-else>
        ...
        <p style="text-align:center;"  @tap="handleToHome">
          <el-button type="danger" size="mini" style="font-size:16px;">点击这里去下单吧</el-button>
        </p>
      </div>
    </Scroller>
  </div>
...
<script>
import { reqDoneOrderList } from "@/api";
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (!isAuth) {
      ...
      next((vm) => {
        ...
        vm.getDoneOrderList(vm.consumerInfo.id.toString());
      });
    }
  },
  methods: {
    async getDoneOrderList(id) {
      const result = await reqDoneOrderList(id);
      if (result.code === 0) {
        this.doneOrderList = result.data;
